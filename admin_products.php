		...
		<?php
			if(isAdmin() == false){
				...
				if($_SERVER['REQUEST_METHOD'] == 'POST'){
					if(isset($_POST['DeleteButton'])){
						...
						$qu = "UPDATE products SET product_approved = 1 WHERE product_id = {$_POST['product_id']}";
						$re = mysqli_query($dbc,$qu);
					}
				}
			}
			global $user;
			$q = "SELECT * FROM products INNER JOIN shopping_users ON products.vendor_id=shopping_users.user_id WHERE product_approved = 0";
			...
			while($p = mysqli_fetch_array($products)){
				...
				echo "<div style=\"text-align:right\" class=\"pull-right\">";
					...
					echo "<form action=\"\" method=\"POST\"><input type=\"text\" name=\"product_id\" value=\"{$p['product_id']}\" hidden><input type=\"submit\" name=\"ApproveButton\" value=\"Approve\" style=\"margin-right:10px;\" class=\"btn btn-success\" ><input type=\"submit\" name=\"DeleteButton\" value=\"Delete\" class=\"btn btn-danger\"></form>";
				echo "</div>";
